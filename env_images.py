...
for training_env in tr_schedule.training_schedule:
    if isinstance(training_env, RoughTerrain):
        ...
        ind.make_screenshot_env(f"{folder_run_data}/rough/rough_{training_env.block_size}_{training_env.floor_height}.pdf")
    elif isinstance(training_env, HillsTerrain):
        ind.setup_ant_hills(params, training_env.floor_height, training_env.scale)
        ind.make_screenshot_env(f"{folder_run_data}/hills/hills_{training_env.scale}_{training_env.floor_height}.pdf")
    elif isinstance(training_env, DefaultTerrain):
        ind.setup_ant_default(params)
        ind.make_screenshot_env(f"{folder_run_data}/default_terrain.pdf")
    else:
